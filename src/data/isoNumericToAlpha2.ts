/**
 * ISO 3166-1 numeric → alpha-2 codes used by world-atlas TopoJSON `id` fields.
 * Only codes present in countries-110m geometries need entries for click/heat mapping.
 */
export const ISO_NUMERIC_TO_ALPHA2: Record<string, string> = {
  ...
  '458': 'MY', '466': 'ML', '478': 'MR', '480': 'MU', '484': 'MX', '496': 'MN',
  ...
  '760': 'SY', '762': 'TJ', '764': 'TH', '768': 'TG', '780': 'TT', '784': 'AE',
  '788': 'TN', '792': 'TR', '795': 'TM', '800': 'UG', '804': 'UA', '807': 'MK',
  '818': 'EG', '826': 'GB', '834': 'TZ', '840': 'US', '854': 'BF', '858': 'UY',
  '860': 'UZ', '862': 'VE', '887': 'YE', '894': 'ZM',
};

/** Also accept unpadded numeric ids from TopoJSON (e.g. "840" vs "840"). */
export function numericIdToAlpha2(id: string | number | undefined): string | null {
  if (id === undefined || id === null) return null;
  const raw = String(id);
  if (ISO_NUMERIC_TO_ALPHA2[raw]) return ISO_NUMERIC_TO_ALPHA2[raw];
