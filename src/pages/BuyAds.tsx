...
const paymentMethods = PAYMENT_METHODS.map(pm => ({
  ...
  name: pm.name,
  sub: pm.subtitle,
  icon: typeof pm.icon === 'string' ? pm.icon : <pm.icon className={`w-6 h-6 ${pm.color} mx-auto`} />,
  color: pm.color,
  border: pm.border,
  bg: pm.bg,
}));
...
Return valid JSON with exactly two fields: "headline" (max 60 characters, punchy and Bitcoin-native) and "description" (max 150 characters, conversion-focused). No extra fields.`,
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto space-y-6">
      ...
      <PaymentModal
        show={showInvoice}
        onClose={() => { if (paymentStatus !== 'processing') setShowInvoice(false); }}
        paymentStatus={paymentStatus}
        bolt11Invoice={bolt11Invoice}
        invoiceSecondsLeft={invoiceSecondsLeft}
